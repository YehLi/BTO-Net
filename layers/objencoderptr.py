...
class ObjEncoderPtr(nn.Module):
    def __init__(self, embed_dim, att_hidden_drop, dropout1, dropout2):
        super(ObjEncoderPtr, self).__init__()
        self.num_layers = 2
        ...
        if cfg.MODEL.BTONET.OBJ_MH == False:
            self.p_att_feats = nn.Linear(embed_dim, embed_dim)
            self.att = ObjAttention(embed_dim, att_hidden_drop)
        else:
            self.att = blocks.create(
                'MultiheadAttention',
                hidden_size = embed_dim, 
                num_attention_heads = cfg.MODEL.TRANSFORMER.HEAD, 
    ...
    def forward(self, encoder_out, obj_seq, att_mask, obj_seq_label=None, final_att_mask=None, ss_prob=0):
        ...
        for t in range(obj_len):
            ...
            if self.training and t >=1 and ss_prob > 0:
                ...
                mask = prob < ss_prob
                if mask.sum() == 0:
                    cur_obj_feat = obj_feat[:, t, :]
                else:
                    ind = mask.nonzero().view(-1)
                    obj_seq_idx = ori_obj_seq[:, t-1].data.clone() + 1
                    prob_prev = F.softmax(mlogit.detach(), dim=-1)#########################prob_prev = torch.exp(mlogit.detach())
                    obj_seq_idx.index_copy_(0, ind, torch.multinomial(prob_prev, 1).view(-1).index_select(0, ind))
                    obj_seq_idx = obj_seq_idx.unsqueeze(-1).expand(batch_size, self.embed_dim)
                    cur_obj_feat =  torch.gather(encoder_out, 1, obj_seq_idx.unsqueeze(1)).squeeze(1)
            else:
                cur_obj_feat = obj_feat[:, t, :]
            ####################################################################################################
            ...
            if cfg.MODEL.BTONET.OBJ_MH == False:
                ...
                att, att_logit = self.att(h1_t.unsqueeze(1), att_feats, att_feats, att_mask[:, 1:].unsqueeze(1))
                att = att.squeeze(1)
                att_logit = att_logit.squeeze(-2).mean(1)
            ...
            if final_att_mask is not None:
                mlogit = mlogit.masked_fill(final_att_mask == 0, -1e9)
            ...
            if cfg.MODEL.BTONET.TRAIN_WITH_TOPK > 1:
                ...
                #############################################################################
                if cfg.MODEL.BTONET.DEC_NO_REPEAT == True:
                    if final_att_mask is None:
                        final_att_mask = torch.zeros_like(att_mask) + 1
                    copy_value = torch.zeros(obj_seq_pred.shape[0], 1).cuda()
                    final_att_mask.scatter_(1, obj_seq_pred.unsqueeze(-1), copy_value)
                    final_att_mask[:,0] = 1
                #############################################################################
        ...
        if cfg.MODEL.BTONET.TRAIN_WITH_GT:
            ...
            outputs = outputs + positions
        return outputs, outputs_h, nce_logit, obj_att_mask, att_logits, None

    def decode(self, encoder_out, att_mask, final_att_mask, **kwargs):
        ...
        outputs_idx = Variable(torch.zeros(batch_size, max_obj_len).cuda())
        ...
        for t in range(max_obj_len):
            ...
            if cfg.MODEL.BTONET.OBJ_MH == False:
                ...
                att, _ = self.att(h1_t.unsqueeze(1), att_feats, att_feats, att_mask[:, 1:].unsqueeze(1))
                att = att.squeeze(1)
            ...
            if final_att_mask is not None:
                scores = scores.masked_fill(final_att_mask == 0, -1e9)
            nce_logit[:, t, :] = scores
            ...
            if cfg.MODEL.BTONET.TRAIN_WITH_TOPK > 1:
                ...
                if greedy_decode:
                    ...
                    n_scores = nn.Softmax(dim=-1)(scores)
                    wt = torch.multinomial(n_scores, 1)

                wt = wt.view(-1).long()
                unfinished = unfinished * (wt > 0)
                outputs_idx[:, t] = wt * unfinished.type(torch.cuda.LongTensor)

                #############################################################################
                if cfg.MODEL.BTONET.DEC_NO_REPEAT == True:
                    if final_att_mask is None:
                        final_att_mask = torch.zeros_like(att_mask) + 1
                    copy_value = torch.zeros(wt.shape[0], 1).cuda()
                    final_att_mask.scatter_(1, wt.unsqueeze(-1), copy_value)
                    final_att_mask[:,0] = 1
                #############################################################################
        ...
        outputs = outputs + positions
        return outputs, obj_att_mask, nce_logit, outputs_idx
