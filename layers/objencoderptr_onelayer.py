...
class ObjEncoderPtrOneLayer(nn.Module):
    def __init__(self, embed_dim, att_hidden_drop, dropout1, dropout2):
        super(ObjEncoderPtrOneLayer, self).__init__()
        self.num_layers = 2
        ...
        # First LSTM layer
        self.lstm1 = nn.LSTMCell(embed_dim * 2, embed_dim)
        ...
        self.ctx_drop = nn.Dropout(dropout1) if dropout1 > 0 else nn.Identity()
    ...
    def forward(self, encoder_out, obj_seq, att_mask, obj_seq_label=None):
        ...
        for t in range(obj_len):
            ...
            h1_t, c1_t = self.lstm1(torch.cat([obj_feat[:, t, :], gv_feat + self.ctx_drop(state[0][1])], 1), (state[0][0], state[1][0]))
            att, att_logit = self.att(h1_t, att_feats, p_att_feats, att_mask[:, 1:])
            att_logits[:,t,:] = att_logit
            ctx_input = torch.cat([att, h1_t], 1)
            h2_t = self.att2ctx(ctx_input)
            state = (torch.stack([h1_t, h2_t]), torch.stack([c1_t, state[1][1]]))
            outputs_h[:, t, :] = h2_t
    ...
    def decode(self, encoder_out, att_mask, **kwargs):
        ...
        for t in range(max_obj_len):
            ...
            #outputs[:, t, :] = h2_t
            h1_t, c1_t = self.lstm1(torch.cat([cur_obj_feat, gv_feat + state[0][1]], 1), (state[0][0], state[1][0]))
            att, _ = self.att(h1_t, att_feats, p_att_feats, att_mask[:, 1:])
            ctx_input = torch.cat([att, h1_t], 1)
            h2_t = self.att2ctx(ctx_input)
            state = (torch.stack([h1_t, h2_t]), torch.stack([c1_t, state[1][1]]))
