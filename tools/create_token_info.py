...
def token_info(annos, phase):
        ...
        for i in range(n):
            ...
            for j, rids in enumerate(tokens2rid[i]):
                if tokens2label[i][j] == -1:
                    ...
                    if len(objects_pos_seq) == 0 or rids != objects_pos_seq[len(objects_pos_seq) - 1]:
                        objects_pos_seq.append(rids)
                        rids_set = set(rids)
    ...
    save_pickle(token_info, os.path.join(args.outfolder, 'sent', 'tokens_' + phase + '_info.pkl'))
